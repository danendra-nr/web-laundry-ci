...
<div class="max-w-2xl bg-white rounded-2xl border border-gray-150 shadow-sm overflow-hidden">
    <div class="px-6 py-5 border-b border-gray-100 flex items-center justify-between">
        <h3 class="text-base font-bold text-gray-800">Edit Transaksi (<?= htmlspecialchars($transaksi['invoice']) ?>)</h3>
    ...
    <form action="<?= base_url('transaksi/update/' . $transaksi['id']) ?>" method="POST" class="p-6 space-y-5">
        <?= csrf_field() ?>
        ...
        <div>
            ...
            <select name="pelanggan_id" id="pelanggan_id" class="w-full px-4 py-2.5 text-sm font-medium border border-gray-200 rounded-xl focus:border-blue-500 focus:ring-2 focus:ring-blue-100 outline-none transition-all bg-white" required>
                ...
                <?php foreach ($pelanggan as $p): ?>
                    <option value="<?= $p['id'] ?>" <?= old('pelanggan_id', $transaksi['pelanggan_id']) == $p['id'] ? 'selected' : '' ?>><?= htmlspecialchars($p['nama']) ?> (<?= htmlspecialchars($p['no_hp']) ?>)</option>
                <?php endforeach; ?>
            </select>
        </div>
        ...
        <div>
            ...
            <select name="layanan_id" id="layanan_id" onchange="updateServiceDetails()" class="w-full px-4 py-2.5 text-sm font-medium border border-gray-200 rounded-xl focus:border-blue-500 focus:ring-2 focus:ring-blue-100 outline-none transition-all bg-white" required>
                ...
                <?php foreach ($layanan as $l): ?>
                    <option value="<?= $l['id'] ?>" <?= old('layanan_id', $transaksi['layanan_id']) == $l['id'] ? 'selected' : '' ?>><?= htmlspecialchars($l['nama_layanan']) ?> (<?= htmlspecialchars($l['kode_layanan']) ?>)</option>
                <?php endforeach; ?>
            </select>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <input type="number" step="0.01" name="berat_kg" id="berat_kg" value="<?= old('berat_kg', floatval($transaksi['berat_kg'])) ?>" placeholder="Contoh: 3.5" class="w-full px-4 py-2.5 text-sm font-medium border border-gray-200 rounded-xl focus:border-blue-500 focus:ring-2 focus:ring-blue-100 outline-none transition-all" required>
            </div>
            ...
            <div>
                ...
                <input type="date" name="tanggal_masuk" id="tanggal_masuk" value="<?= old('tanggal_masuk', $transaksi['tanggal_masuk']) ?>" class="w-full px-4 py-2.5 text-sm font-medium border border-gray-200 rounded-xl focus:border-blue-500 focus:ring-2 focus:ring-blue-100 outline-none transition-all bg-white" required>
            </div>
        </div>

        <div>
            <label for="catatan" class="block text-sm font-semibold text-gray-700 mb-1.5">Catatan Khusus</label>
            <textarea name="catatan" id="catatan" rows="3" placeholder="Contoh: Baju putih dipisah, luntur, dll..." class="w-full px-4 py-2.5 text-sm font-medium border border-gray-200 rounded-xl focus:border-blue-500 focus:ring-2 focus:ring-blue-100 outline-none transition-all"><?= old('catatan', $transaksi['catatan']) ?></textarea>
        </div>
